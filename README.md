# Valyd Developer Portal — Admin Console (`admin.valyd.work`)

Back-office for the **dev.valyd.work** developer portal. Admins sign in with
...
- Backend: new `/api/admin/*` blueprint inside the existing **dev.valyd.work**
...
## Deploy (production)
DNS for `admin.valyd.work` already points at this server.
```bash
npm run build
sudo bash deploy/go-live.sh   # installs nginx site + certbot TLS, reloads nginx
```
`deploy/admin.valyd.work.nginx` serves `dist/` and proxies `/api/` to the
